    ...
    return PassMarker.useMobileLayout!
        ...
        : AlertDialog(
            ...
            content: Column(mainAxisSize: MainAxisSize.min, children: <Widget>[
              ...
              Text('Owner: ' + PassMarker.namePopup,
                  style: TextStyle(
                      fontSize: screenText * 24, fontWeight: FontWeight.bold)),
              SizedBox(height: screenHeight * 0.01),
              _buildRow(
                  'assets/choc.png',
                  PassMarker.carModel.vehicle.toString(),
                  'VEHICLE',
                  screenWidth,
                  screenHeight,
                  screenText),
              _buildRow('assets/choc.png', PassMarker.carModel.model.toString(),
                  'MODEL', screenWidth, screenHeight, screenText),
              _buildRow('assets/choc.png', PassMarker.carModel.fuel.toString(),
                  'FUEL', screenWidth, screenHeight, screenText),
              _buildRow('assets/choc.png', PassMarker.carModel.seats.toString(),
                  'SEATS', screenWidth, screenHeight, screenText),
              _buildRow('assets/choc.png', PassMarker.carModel.price.toString(),
                  'PRICE FOR DAY', screenWidth, screenHeight, screenText),
              SizedBox(height: screenHeight * 0.02),
              FloatingActionButton(
                  onPressed: () async {
                    showDialog(
                    ...
                    List<String> files = await urlFile(
                        PassMarker.carModel.uid!, PassMarker.carModel.cid!);
                ...
                child: MaterialButton(
                    ...
                    onPressed: () async {
                      if (await NetworkCheck().check()) {
                        ...
                        if (await _isConfirmed(user!) == 'confirmed') {
                          PassMarker.hpOrNot = true;
                          var reserveResult = 'start';
                          reserveResult = await Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) =>
                                      Least(service: Service())));

                          if (reserveResult == '1') {
                            Navigator.of(context, rootNavigator: true)
                                .pop('dialog');
                            Fluttertoast.showToast(
                                msg: 'Your reservation has been approved :)',
                                fontSize: 20);
                    ...
                    child: Text("Reserve",
                        style: TextStyle(
                            color: Colors.white, fontSize: screenText * 28))),
              ),
            ]));
  }
